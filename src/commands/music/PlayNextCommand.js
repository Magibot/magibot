...
class PlayNextCommand extends Commando.Command {
    ...
    constructor(client) {
        super(client, {
            ...
            group: "music",
            memberName: "playnext",
            description: "Insere a próxima a ser tocada na primeira posição da fila."
        });
    }
    ...
    async run(msg, args) {
        ...
        if (!args) {
            return msg.channel.send(`Insira um parametro de pesquisa valido. Exemplo: \`!playnext <insira-link>\``);   
        }
        ...
        if (global.servers[msg.guild.id].queue.length > 1) {
            answer = new Discord.RichEmbed()
                ...
                .setURL(newSong.url)
                .setAuthor(`Próxima a tocar`, msg.member.user.avatarURL)
